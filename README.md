...
### C端小程序功能
- **用户系统**: 微信授权登录、手机号登录、个人资料管理、照片上传
- **实名认证**: 实名/单身承诺/学历/房产/车产/到店认证（支持腾讯云 E 证通 OCR）
- **缘分匹配**: AI 缘分匹配、每日推荐、筛选、访客记录/谁看过我、互相喜欢
- **动态社区**: 用户动态、红娘动态、点赞互动
- **问答社区**: 热门问答、回答互动、点赞
- **红娘服务**: 专属红娘、红娘牵线、红娘评语、AI 红娘
- **会员体系**: 会员订阅（月度/季度套餐，按时长划分，无等级差异）
- **人格测试**: 性格测试、性格海报分享
- **活动/圈子**: 最新活动报名、相亲圈子、成功案例
- **分享海报**: 个性化分享海报生成与追踪
- **系统消息**: 通知消息、系统消息聚合

> 注：为通过微信审核（个体户非社交类目），小程序端已移除站内私信聊天入口，并将「喜欢/关注/想认识Ta/红娘牵线/回答问题」等按钮做成管理后台开关，关闭后前端不展示对应入口（详见下方「功能开关」章节）。
...
为满足微信小程序「个体户非社交类目」审核要求，小程序端弱化交友/社交属性，改为「婚庆展示信息」定位。以下入口/按钮均做成管理后台开关（系统配置 → 基础配置 → 功能开关），关闭后前端不再展示对应入口：

| 开关 | 控制内容 |
|------|----------|
| 聊天功能 | 关闭后所有用户无法发送聊天消息（小程序端已移除私信入口） |
| VIP 会员 | 关闭后会员页不显示 |
| 去留言按钮 | 互相喜欢页「去留言」按钮显隐 |
| 喜欢按钮 | 首页用户卡片「喜欢/心动」按钮显隐 |
| 关注按钮 | 用户详情页「关注」按钮显隐 |
| 想认识Ta按钮 | 用户详情页及动态页卡片「想认识Ta」按钮显隐 |
| 红娘牵线按钮 | 用户详情页及动态页卡片「红娘牵线」按钮显隐 |
| 回答问题按钮 | 问答页「回答」按钮显隐 |
| 专属红娘入口 | 我的页面「专属红娘」入口显隐 |
| 我的喜欢入口 | 我的页面「我的喜欢」入口显隐 |
| 我的性格入口 | 我的页面「我的性格/测一测」入口显隐 |
...
│   └── package.json
│
├── src/                      # 小程序（uni-app，微信小程序）
│   ├── pages/                # 主包页面（首页/动态/会员/消息/我的等）
│   ├── subpkg-pages/         # 分包页面（认证/问答/活动/设置/人格测试等）
│   ├── components/           # 公共组件（tab-bar/弹窗/卡片等）
│   ├── store/                # Pinia 状态管理（user/system/message/filter）
│   ├── composables/          # 组合式函数
│   ├── utils/                # 请求/鉴权/日志/导航/上传等工具
