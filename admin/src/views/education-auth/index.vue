<template>
  <div class="education-auth-list">
    <div class="page-header">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/dashboard' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item>审核管理</el-breadcrumb-item>
        <el-breadcrumb-item>学历认证审核</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <div class="card">
      <el-table v-loading="loading" :data="list" stripe>
        ...
        <el-table-column label="毕业院校" min-width="160" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.authData?.school || '-' }}
          </template>
        </el-table-column>
...
const list = ref<EducationAuthItem[]>([])
const total = ref(0)
...
const currentRejectRow = ref<EducationAuthItem | null>(null)
...
async function fetchList() {
  ...
  try {
    ...
    const res = await adminEducationAuth.list(params)
    const data = res.data
...
function handleApprove(row: EducationAuthItem) {
  ElMessageBox.confirm(`确认通过用户「${row.user?.nickname || row.userId}」的学历认证审核？`, '审核确认', {
...
function handleReject(row: EducationAuthItem) {
...
async function confirmReject() {
  ...
  try {
    await adminEducationAuth.audit(currentRejectRow.value.id, {
      status: 2,
...
function handleReaudit(row: EducationAuthItem) {
  ElMessageBox.confirm(`确认重新审核用户「${row.user?.nickname || row.userId}」的学历认证？`, '重新审核确认', {
    confirmButtonText: '确认',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(async () => {
    try {
      await adminEducationAuth.audit(row.id, { status: 0 })
