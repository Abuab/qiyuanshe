...
// 互动记录折叠面板 active（非手风琴模式，允许多面板同时展开，使用数组类型）
const interactionActivePanels = ref<string[]>([])
...
// ===== 审核记录辅助函数 =====
/** 审核状态标签映射：后端返回字符串('PENDING'/'APPROVE'/'REJECT'/'unsubmitted')，detail API 不返回(undefined→未提交) */
function getAuditStatusLabel(status?: number | string) {
  if (status === undefined || status === null || status === 'unsubmitted') return '未提交'
  const labelMap: Record<string, string> = { PENDING: '待审核', APPROVE: '已通过', REJECT: '已拒绝' }
  if (typeof status === 'number') return { 0: '未提交', 1: '待审核', 2: '已通过', 3: '已拒绝' }[status] || '未知'
  return labelMap[status] || status
}
function getAuditTagType(status?: number | string) {
  if (status === undefined || status === null || status === 'unsubmitted') return 'info'
  const typeMap: Record<string, string> = { PENDING: 'warning', APPROVE: 'success', REJECT: 'danger' }
  if (typeof status === 'number') return { 0: 'info', 1: 'warning', 2: 'success', 3: 'danger' }[status] || 'info'
