 ...
 * 运行方式：
 *   cd backend
 *   npx ts-node src/scripts/seed-streets.ts
 *
 * 或通过 npm script（需先在 package.json 添加）：
...
async function main() {
  ...
  for (let i = 0; i < streets.length; i += BATCH_SIZE) {
    ...
    const values = batch
      .map((s) => {
        const id = parseInt(s.code, 10) // 12 位完整代码
        const parentId = parseInt(s.areaCode, 10) // 区县代码
    ...
    try {
      ...
      await ds.query(sql)
      // 批量 INSERT IGNORE 无法精确获取插入条数，按批次大小估算
